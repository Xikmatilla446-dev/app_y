...
import { useContentGetMenuQ } from "@/hooks/queries";
...
const Navbar = () => {
	...
	const { data, isLoading } = useContentGetMenuQ({
		categoryId: "1",
		options: {
			onSettled(data) {
				setSubMenu(get(data, "data", []));
			}
		}
	});
								...
								{item?.child?.length > 0 ? (
									<div
										...
										<span className={classes.innerWithIcon}>
											{getNameWithLang(item, "name_", locale)}
											<ArrowDown />
										</span>
										...
										{item?.child?.length > 0 ? (
											<div
												...
												<ul className={classes.sub_menu}>
													{item?.child?.map((sub: any, indx: number) => (
														<li key={indx}>
															<Link href={`${sub.path}`}>{getNameWithLang(sub, "name_", locale)}</Link>
														</li>
													))}
												</ul>
											</div>
									...
									<Link href={`${item?.path ?? ""}`} key={k} className={classes.link}>
										<span className={classes.inner}>{getNameWithLang(item, "name_", locale)}</span>
										{item?.child?.length > 0 ? (
											<div className={classNames(classes.sub_menu_wrap, item.sub === 2 ? classes.sub : "")}>
												<ul className={classes.sub_menu}>
													{item?.child?.map((sub: any, indx: number) => (
